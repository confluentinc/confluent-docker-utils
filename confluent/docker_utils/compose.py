...
class ComposeContainer:
    ...
    def name_without_project(self) -> str:
        ...
        return self.container.labels.get(LABEL_SERVICE) or self.name.split('_')[1] if '_' in self.name else self.name
    ...
    def exit_code(self) -> Optional[int]:
        self.container.reload()
        return self.container.attrs['State']['ExitCode'] if self.container.status == 'exited' else None
...
class ComposeProject:
    ...
    def _build_config(self, svc: Dict) -> Dict:
        ...
        if 'environment' in svc:
            env = svc['environment']
            cfg['environment'] = dict(e.split('=', 1) for e in env) if isinstance(env, list) else env
        
        if 'ports' in svc:
            cfg['ports'] = {str(p).split(':')[1]: int(str(p).split(':')[0]) 
                          for p in svc['ports'] if ':' in str(p)}
