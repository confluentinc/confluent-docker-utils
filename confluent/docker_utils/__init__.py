...
def build_image(image_name: str, dockerfile_dir: str):
    ...
    _, logs = api_client().images.build(path=dockerfile_dir, rm=True, tag=image_name)
    for line in logs:
        if 'stream' in line:
...
def run_docker_command(timeout=None, **kwargs) -> bytes:
    ...
    cfg = {
        ...
        'labels': {"io.confluent.docker.testing": "true"},
        'detach': True,
    }
    
    host_cfg = kwargs.get('host_config', {})
    if 'NetworkMode' in host_cfg:
        cfg['network_mode'] = host_cfg['NetworkMode']
    if 'Binds' in host_cfg:
        cfg['volumes'] = {b.split(':')[0]: {'bind': b.split(':')[1], 'mode': 'rw'} 
                         for b in host_cfg['Binds']}
